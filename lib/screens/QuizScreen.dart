...
class _QuizScreenState extends State<QuizScreen> {
  ...
  Future<List<String>> fetchCategoriesFromFirestore() async {
    ...
    // Fetch all the documents from the 'questions' collection
  ...
  void _handleMessage(RemoteMessage message) {
    ...
    if (clickAction != null) {
      if (clickAction == 'FLUTTER_NOTIFICATION_CLICK') {
        // Extract the questionId from the FCM message data
  ...
  void _loadFavoriteQuestions() {
    final favorites = _prefs.getStringList('favoriteQuestions');
    if (favorites != null) {
      setState(() {
        _favoriteQuestions = favorites.map(int.parse).toSet().toList();
      });
    }
  }
              ...
              children: categories.map((category) {
                return ListTile(
                    ...
                    onTap: () async {
                      ...
                      final questionsData = await fetchQuestionsFromFirestoreByCategory(standardizedCategoryValue);

                      // Fetch questions based on category.

                      setState(() {
                        _questions = questionsData.map((data) => data['question'] as String).toList();
                        _answers = questionsData.map((data) => List<String>.from(data['answers'])).toList();
                      ...
                      // Load favorite questions after updating the state
                      _loadFavoriteQuestions();

                      // Navigate to a new screen where all questions, correct answers, and explanations are shown.
                      Navigator.push(
                        context,
                        MaterialPageRoute(
  ...
  Future<void> addFavoriteQuestion(String userId, List<int> favoriteQuestions) async {
    final firestore = FirebaseFirestore.instance;
...
//gurumohan
  void _showFavoriteQuestions() {
    // Pass the favorite questions and explanations to the FavoriteQuestionsScreen
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => FavoriteQuestionsScreen(
          favoriteQuestions: _favoriteQuestions.map((index) => _questions[index]).toList(),
  ...
  Future<void> fetchUserFavorites(String userId) async {
    ...
    if (userData.exists) {
      ...
      final favoriteQuestions = data['favoriteQuestions'] as List<dynamic>;
      setState(() {
        _favoriteQuestions = favoriteQuestions.cast<int>().toList();
      });
    }
  }
  ...
  Future<List<Map<String, dynamic>>> fetchQuestionsFromFirestoreByCategory([String? category]) async {
    ...
    try {
      Query query = firestore.collection('questions').orderBy('timestamp', descending: true);
      if (category != null) {
  ...
  void _shareApp() async {
    ...
    final String questionText = _questions[_currentQuestionIndex]; // Get the current question text
  ...
  void _toggleFavorite(int questionIndex) {
    ...
    _prefs.setStringList('favoriteQuestions', _favoriteQuestions.map((index) => index.toString()).toList());
