...
class CurrentAffairsScreen_te extends StatefulWidget {
  @override
  _CurrentAffairsScreen_teState createState() => _CurrentAffairsScreen_teState();
}

class _CurrentAffairsScreen_teState extends State<CurrentAffairsScreen_te> {
  List<String?> titles = [];
  ...
  void _saveFavorites() async {
    ...
    await prefs.setStringList('favoriteArticles_te', validIndices.map((index) => newsArticles[index]).toList());
  }

  void _loadFavorites() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    List<String> storedFavorites = prefs.getStringList('favoriteArticles_te') ?? [];

    // Assuming your newsArticles_te are loaded and ready at this point
  ...
  Future<List<Map<String, dynamic>>> fetchCurrent_affairs_teFromFirestoreByCategory([String? category]) async {
    ...
    try {
      Query query = firestore.collection('current_affairs_te').orderBy('timestamp', descending: true);
      if (category != null) {
      ...
      print('Error fetching current_affairs_te: $e');
      return [];
    }
  }
  ...
  Future<List<String>> fetchCategoriesFromFirestore() async {
    ...
    final querySnapshot = await firestore.collection('current_affairs_te').get();
  ...
  Future<void> fetchCurrentAffairsData() async {
    try {
      final articlesData = await fetchCurrent_affairs_teFromFirestoreByCategory();
      setState(() {
        _loadFavorites();
              ...
              children: categories.map((category) {
                return ListTile(
                  ...
                  onTap: () async {
                    ...
                    final articlesData = await fetchCurrent_affairs_teFromFirestoreByCategory(standardizedCategoryValue);
                    print('Selected Category: $standardizedCategoryValue');
          ...
          IconButton(
            ...
            onPressed: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => FavoritesScreen_te(
                    favoriteArticles_te: favoriteArticles.map((index) => newsArticles[index]).toList(),
                  ),
                ),
              );
            },
...
class FavoritesScreen_te extends StatelessWidget {
  ...
  FavoritesScreen_te({required this.favoriteArticles_te});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ListView.builder(
        itemCount: favoriteArticles_te.length + 1, // Add 1 for the banner ad
        itemBuilder: (context, index) {
          if (index == 0) {
            ...
            return ListTile(
              title: _buildBoldFirstLineText(favoriteArticles_te[index - 1]),
